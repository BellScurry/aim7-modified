...
#include <string.h>
#include "aim7.h"			/* our goodies */

enum choices { STAT = 0, CREAT, MCHOICE };	/* encode choices here */

#define MFILES	1000   /*75		* number of entries */
...
static int dsearch(const char *fakeh_dir);
static int get_list(FILE * file, char *list[MCHOICE][MFILES]);
static void scramble(char *list[], int num);
static unsigned long mrand();
static void clear_list(char *list[MCHOICE][MFILES]);
static void errdump(int line, char *str);

static int dsearch(const char *fakeh_dir) {
	
    FILE *fp;			/* file containing filenames */

	int fd,				/* file discriptor for creat, etc. */
	  index;			/* loop variable */

	struct stat stbuf;		/* stat buffer */

	char cwd[256],			/* hold current working dir */
...
static int get_list(FILE * file, char *list[MCHOICE][MFILES]) {
    ...
    char buff[MYBUF],			/* holds 1 line of input */
	 *tmp;				/* holds malloc results */

	int s_index, c_index, i;
	...
	while (fgets(buff, MYBUF - 1, file) != NULL) {	/* get a line */
		...
		if ((tmp = malloc(strlen(buff) + 1 + 8)) == NULL) {	/* allocate space */
			clear_list(list);	/* handle bad allocate */
			return (-1);	/* return error */
		}
		/*
...
static unsigned long mrand() {					/* return integer randome number */
